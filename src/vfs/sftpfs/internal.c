...
static int
sftpfs_internal_stat (sftpfs_super_data_t ** super_data, const vfs_path_element_t ** path_element,
...
sftpfs_lstat (const vfs_path_t * vpath, struct stat *buf, GError ** mcerror)
{
    ...
    res =
        sftpfs_internal_stat (&super_data, &path_element, vpath, mcerror, LIBSSH2_SFTP_LSTAT,
                              &attrs);
    if (res >= 0)
    {
...
sftpfs_stat (const vfs_path_t * vpath, struct stat *buf, GError ** mcerror)
{
    ...
    res =
        sftpfs_internal_stat (&super_data, &path_element, vpath, mcerror, LIBSSH2_SFTP_STAT,
                              &attrs);
    if (res >= 0)
    {
        buf->st_nlink = 1;
